...
orax-cli-windows-386.exe:
	env GOOS=windows GOARCH=386 go build -ldflags $(LDFLAGS_PROD) -o $(PROD_BUILD_FOLDER)/orax-cli-windows-386-$(REVISION).exe
...
orax-cli-staging-windows-386.exe:
	env GOOS=windows GOARCH=386 go build -ldflags $(LDFLAGS_STAGING) -o $(STAGING_BUILD_FOLDER)/orax-cli-staging-windows-386-$(REVISION).exe
